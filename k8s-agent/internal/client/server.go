...
// NewServerClient creates a client with mTLS configured when cert/key PEMs are
// provided. Pass empty strings for the PEM arguments when the client is used
// only for the initial registration request.
...
// RegistrationRequest is sent by the agent when registering with the server.
...
// RegistrationResponse contains the mTLS certificates and API token issued by the server.
...
// Register sends a registration request to the server and returns the issued credentials.
...
// ConnectWebSocket dials the server WebSocket endpoint and authenticates as an agent.
